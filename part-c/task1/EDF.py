...
# Task4 the softmax and cross-entropy nodes
class Softmax(Node):
    def __init__(self, node):
        Node.__init__(self, [node])
...
class CrossEntropy(Node):
    ...
    def forward(self):
        y_true, y_pred = self.inputs
        self.value = -np.sum(y_true.value * np.log(y_pred.value +  1e-10)) / y_true.value.shape[0]

    def backward(self):
        y_true, y_pred = self.inputs
        self.gradients[y_pred] = (-y_true.value / (y_pred.value +  1e-10)) / y_true.value.shape[0]
